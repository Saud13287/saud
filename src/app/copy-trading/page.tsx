...
function buildExpertRankings(): ExpertRanking[] {
  ...
  return agentRegistry.map((expert) => {
    ...
    return {
      id: expert.id,
      nameAr: expert.nameAr,
      nameEn: expert.nameEn,
      icon: expert.icon,
      category: categories[expert.category] || expert.category,
      accuracy: expert.accuracy,
      weight: expert.weight,
      totalSignals: 50 + (seed * 3) % 200,
      correctSignals: 40 + (seed * 2) % 150,
      totalPnl: Math.round(((seed % 100) / 100 - 0.3) * 50000),
      avgConfidence: 80 + (seed % 15),
      trend: (seed % 3 === 0 ? "up" : seed % 3 === 1 ? "stable" : "down") as "up" | "down" | "stable",
      bestAsset: assets[seed % assets.length],
    };
...
export default function CopyTradingPage() {
  ...
  const toggleCopyExpert = (expertId: string) => {
    setCopySettings((prev) => prev.map((s) =>
      s.expertId === expertId ? { ...s, enabled: !s.enabled } : s
    ));
  };

  const updateCopySetting = (expertId: string, updates: Partial<CopySetting>) => {
    setCopySettings((prev) => prev.map((s) =>
      s.expertId === expertId ? { ...s, ...updates } : s
    ));
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-[var(--color-omega-muted)]">
            {enabledCount} خبير مُفعّل للنسخ | {expertRankings.length} خبير متاح
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-[var(--color-omega-card)] px-3 py-1.5 rounded-lg">
            ...
            <button onClick={() => setAutoCopy(!autoCopy)}
              className={`w-8 h-4 rounded-full transition-colors ${autoCopy ? "bg-emerald-500" : "bg-gray-600"}`}>
              <div className={`w-3 h-3 rounded-full bg-white transition-transform ${autoCopy ? "translate-x-4" : "translate-x-0.5"}`} />
            </button>
          </div>
          <div className="flex items-center gap-2 bg-[var(--color-omega-card)] px-3 py-1.5 rounded-lg">
            <label className="text-xs">المخاطرة العامة:</label>
            <select value={globalRisk} onChange={(e) => setGlobalRisk(parseFloat(e.target.value))}
              className="bg-[var(--color-omega-surface)] border border-[var(--color-omega-border)] rounded px-2 py-1 text-xs">
              <option value={0.5}>0.5x (منخفض)</option>
              <option value={1.0}>1.0x (عادي)</option>
              <option value={1.5}>1.5x (مرتفع)</option>
              <option value={2.0}>2.0x (جريء)</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {topExperts.map((expert, idx) => (
          <div key={expert.id} className={`bg-[var(--color-omega-card)] border rounded-xl p-4 ${
            idx === 0 ? "border-amber-600/40" : idx === 1 ? "border-gray-400/30" : "border-amber-800/30"
          }`}>
            ...
            <div className="grid grid-cols-2 gap-2 text-center">
              ...
              <div className="bg-[var(--color-omega-surface)] rounded-lg p-2">
                <p className="text-[9px] text-[var(--color-omega-muted)]">الإشارات</p>
                <p className="text-sm font-bold">{expert.totalSignals}</p>
              </div>
            </div>
          </div>
        ))}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-[var(--color-omega-surface)]">
              {["الخبير", "التصنيف", "الدقة", "الوزن", "الإشارات", "P&L", "الثقة", "الأصل الأفضل", "الاتجاه", "النسخ", "إعدادات"].map((h) => (
                ...
                <tr key={expert.id} className="border-t border-[var(--color-omega-border)] hover:bg-[var(--color-omega-surface)]">
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-medium">{expert.nameAr}</p>
                        <p className="text-[9px] text-[var(--color-omega-muted)]">{expert.nameEn}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className={`px-3 py-2 font-bold font-mono ${expert.totalPnl >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                    ${expert.totalPnl.toLocaleString()}
                  </td>
                  <td className="px-3 py-2">{expert.avgConfidence}%</td>
                  <td className="px-3 py-2">{expert.bestAsset}</td>
                  <td className="px-3 py-2">
                    <span className={`text-[9px] ${expert.trend === "up" ? "text-emerald-400" : expert.trend === "down" ? "text-red-400" : "text-amber-400"}`}>
                      {expert.trend === "up" ? "▲ صاعد" : expert.trend === "down" ? "▼ هابط" : "— مستقر"}
                    </span>
                  </td>
                  <td className="px-3 py-2">
                    <button onClick={() => toggleCopyExpert(expert.id)}
                      className={`px-2 py-1 rounded text-[10px] font-bold transition-colors ${
                        setting?.enabled ? "bg-emerald-600 text-white" : "bg-[var(--color-omega-surface)] text-[var(--color-omega-muted)]"
                      }`}>
                  ...
                  <td className="px-3 py-2">
                    {setting?.enabled && (
                      <div className="flex items-center gap-1">
                        <select value={setting.riskMultiplier} onChange={(e) => updateCopySetting(expert.id, { riskMultiplier: parseFloat(e.target.value) })}
                          className="bg-[var(--color-omega-surface)] border border-[var(--color-omega-border)] rounded px-1 py-0.5 text-[9px] w-12">
