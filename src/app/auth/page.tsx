      ...
      <div className="space-y-4 max-w-md mx-auto">
        <div className="bg-[var(--color-omega-card)] border border-[var(--color-omega-border)] rounded-xl p-6 text-center">
          ...
          <div className="flex items-center justify-center gap-2 mt-3">
            <span className="text-[10px] bg-emerald-900/30 text-emerald-400 px-2 py-1 rounded">
          ...
          <button onClick={logout} className="mt-4 bg-red-900/30 hover:bg-red-900/50 text-red-400 px-4 py-2 rounded-lg text-xs transition-colors">
            تسجيل الخروج
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4 max-w-md mx-auto">
        ...
        {mode === "register" && (
          <div>
            ...
            <input type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)}
              placeholder="أدخل اسمك" className="w-full bg-[var(--color-omega-surface)] border border-[var(--color-omega-border)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500" dir="rtl" />
          </div>
        )}
        <div>
          <label className="text-xs text-[var(--color-omega-muted)] block mb-1">البريد الإلكتروني</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            placeholder="example@email.com" className="w-full bg-[var(--color-omega-surface)] border border-[var(--color-omega-border)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500" dir="ltr" />
        </div>
        <div>
          <label className="text-xs text-[var(--color-omega-muted)] block mb-1">كلمة المرور</label>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
            placeholder="••••••••" className="w-full bg-[var(--color-omega-surface)] border border-[var(--color-omega-border)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500" dir="ltr" />
        </div>

        {error && (
          <div className="bg-red-900/20 border border-red-800/30 rounded-lg p-2 text-xs text-red-400">{error}</div>
        )}

        <button onClick={handleSubmit} disabled={loading}
          className="w-full bg-emerald-600 hover:bg-emerald-700 py-2.5 rounded-lg text-sm font-bold transition-colors disabled:opacity-50">
          {loading ? "جاري المعالجة..." : mode === "login" ? "دخول" : "تسجيل"}
        </button>

        <button onClick={() => setMode(mode === "login" ? "register" : "login")}
          className="w-full text-xs text-emerald-400 hover:text-emerald-300 transition-colors">
      ...
      <div className="bg-blue-900/20 border border-blue-800/30 rounded-xl p-4 text-xs text-blue-300">
        ...
        <p>كلمات المرور مشفرة بـ HMAC-SHA256. بياناتك محفوظة في قاعدة بيانات آمنة.</p>
      </div>
    </div>
  );
}
